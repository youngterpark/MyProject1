...
int main_pointer(void)
{
	...
	//int * 미션맨; // 포인터 변수 // 포인터 변수로 주소도 알아올 수 있고
	//미션맨 = &철수;
	...
	//// 두 번째 미션 : 각 암호에 3을 곱해라 // 암호도 바꿀 수 있다.
	//미션맨 = &철수;
	//*미션맨 = *미션맨 * 3;
	...
	//}
	//ptr[0] = 100; // 실행하면 arr 값도 바뀜. 포인터란 녀석이 배열의 값도 찾아가서 바꿈.
	//ptr[1] = 200;
	//ptr[2] = 300;
	//for (int i = 0; i < 3; i++)
	//{
	...
	// 배열일 때, arr2 -> 주소
	int arr2[3] = { 10, 20, 30 }; // 배열의 30을 50으로 바꾸고 싶음.
	changeArray(&arr2[0]);
	for (int i = 0; i < 3; i++)
	{
		printf("%d\n",arr2[i]);
...
void changeArray(int* ptr) // 모르겠음. 포인터에 대한 기본 개념..?
{
	ptr[2] = 50;
}